 ...
 * - Issue #6: web-search 0.9.0 bumped in package.json but never published to npm
 *   because the failed publish consumed the changeset, leaving no trigger for a
 *   retry. Self-healing detects the unpublished version and republishes it.
 * - Reference: js-ai-driven-development-pipeline-template issue #36
 */
...
function getPackageInfo() {
  const packageJson = JSON.parse(
    readFileSync(getPackageJsonPath({ jsRoot }), 'utf8')
  );
