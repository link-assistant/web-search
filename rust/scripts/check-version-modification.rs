...
//!
//! Usage: rust-script rust/scripts/check-version-modification.rs --rust-root rust
//!
...
fn main() {
    ...
    if has_version_change(&diff) {
        ...
        eprintln!(
            "To trigger a release, add a changelog fragment to {}/changelog.d/",
            rust_root
        );
        eprintln!("with the appropriate bump type (major, minor, or patch).\n");
        eprintln!(
            "See {}/changelog.d/README.md for more information.\n",
            rust_root
        );
        eprintln!("If you need to undo your version change, run:");
        eprintln!("  git checkout origin/main -- {}", cargo_toml_path);
        exit(1);
    }
