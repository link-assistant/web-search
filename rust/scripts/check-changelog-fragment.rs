...
//!
//! Usage: rust-script rust/scripts/check-changelog-fragment.rs --rust-root rust
//!
...
fn is_source_file(file_path: &str, rust_root: &str) -> bool {
    ...
    let source_patterns = [
        ...
        Regex::new(&format!(r"^{}Cargo\.lock$", regex::escape(&prefix))).unwrap(),
    ];
...
fn changelog_dir(rust_root: &str) -> String {
    if rust_root == "." {
...
fn is_changelog_fragment(file_path: &str, rust_root: &str) -> bool {
    let changelog_dir = changelog_dir(rust_root);
...
fn main() {
    ...
    if source_changed_count > 0 && fragment_added_count == 0 {
        let changelog_dir = changelog_dir(&rust_root);
        eprintln!("::error::No changelog fragment found in this PR. Please add a changelog entry in {changelog_dir}");
        eprintln!();
        eprintln!("To create a changelog fragment:");
        eprintln!("  Create a new .md file in {changelog_dir} with your changes");
        eprintln!();
        eprintln!("See {changelog_dir}README.md for more information.");
        exit(1);
    }
